...
object FileReceiver {
    ...
    fun cancelAllTransfers(context: Context) {
        ...
        Log.d("FileReceiver", "Cancelling ${transferIds.size} active transfers")
            ...
            try {
                ...
                synchronized(state) {
                    state.pfd?.fileDescriptor?.let { fd ->
                        ...
                        state.receivedBytes += bytes.size.toLong()
                        state.index = index
                    }
                }
    ...
    private fun updateProgressNotification(context: Context, id: String, state: IncomingFileState) {
        ...
        if (timeDiff >= 1.0) {
            ...
            val intervalSpeed = if (timeDiff > 0) bytesDiff.toDouble() / timeDiff else 0.0
            ...
            if (newSpeed > 0) {
                val remainingBytes = (state.size - state.receivedBytes).coerceAtLeast(0L)
            ...
            val percent = if (state.size > 0L) (state.receivedBytes * 100 / state.size) else 0L
        ...
        } else if (state.receivedBytes == 0L) {
            // Initial
            NotificationUtil.showFileProgress(
                context,
                id.hashCode(),
